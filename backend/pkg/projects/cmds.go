...
func ComposeUp(ctx context.Context, proj *types.Project, services []string, removeOrphans bool, forceRecreate bool, recreateVolumes bool, authConfigs map[string]registry.AuthConfig, waitTimeout time.Duration) error {
	...
	var extraOptions []compose.Option
	if envOpts.ParallelLimit > 0 {
		extraOptions = append(extraOptions, compose.WithMaxConcurrency(envOpts.ParallelLimit))
	}

	c, err := NewClient(composeCtx, "", authConfigs, prompt, extraOptions...)
	if err != nil {
		return err
	}
	defer func() { _ = c.Close() }()
